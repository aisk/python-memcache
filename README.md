...
- Every method that stores a value takes a required `ttl` in seconds. There is no client wide default TTL. `FOREVER` (0) stores without expiration and spells that choice out at the call site; a negative ttl is an error. Lifetimes beyond 30 days are converted for you (the raw protocol would reinterpret them as unix timestamps).
